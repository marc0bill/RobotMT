...
On rappelle que le but du projet est de permettre au Robot de se déplacer vers un objet de notre choix et d'éviter d'autres objets.
Ce dernier doit donc être capable d'identifier et de reconnaitre ces objets en temps réel. Pour cela on utilise un algorithme de reconnaissance d'images en script python.


## La librairie Tensorflow

L'algorithme de reconnaissance d'images utilise la librairie Tensorflow. C'est une librairie créée par Google qui propose une série de modèles pré-entraînés. Ces modèles constituent une base de données qui va permettre de différencier et de classifier les différentes images extraites du flux vidéo de la caméra.
...
La reconnaissance d'images se fait en utilisant un modèle pré-entraîné MobileNet couplé avec SSD (Single-Shot multibox Detector), implémenté dans Tensorflow. 
...
SSDLite  utilise des couches séparables en profondeur au lieu de convolutions régulières pour la partie de détection d'objet du réseau. Avec SSDLite, on peut facilement obtenir des résultats véritablement en temps réel.

## L'algorithme de reconnaissance d'images
...
On réalise la détection avec en entrée une image (une frame) issue de la caméra et en sortie on récupère les boxs, les classes et les scores associés dans des tableaux.

Chaque box représente une partie de l'image où un objet à été détecté. Les classes renvoient la nature de l'objet et les scores représentent le niveau de confiance de la classe associée à l'objet détecté.

Nous avons rajouté ce bout de code au script initial afin de retourner une liste de liste contenant, pour chaque objet détecté sur l'image : la classe, le score ainsi que ses coordonnées sur l'image (Ymax, Xmax, Ymin, Xmin)
...
```
    ...
    return (objets[0:c])
```
Le but de ce bout de code est de retourner les valeurs dans un certain ordre afin qu'elles puissent être récupérées pour le contrôle moteur.

## Axes d'améliorations

- Il est possible de créer son propre modèle et de l'entraîner mais cela demande beaucoup de ressources en image (au moins une centaine d'images pour un objet), de temps et c'est une démarche très fastidieuse.

- On peut utiliser la librairie Tensorflowlite, qui a été crée pour les télephones portables ou tout autre appareil portable. Les modèles sont beaucoup moins volumineux et plus rapides (on réduit la taille des poids synaptiques). 
Cependant, les modèles sont donc moins précis que les modèles classiques. De plus, le code de reconnaissance d'images pour cette libraire est différent de celui utilisé pour la libraire Tensorflow.

- Utiliser Intel® Neural Compute Stick : il s'agit d'un dispositif branché sur un port USB qui permet d'accélérer la détection d'images sur RaspberryPi. On peut ainsi charger des modèles volumineux avec la RaspberryPi sans trop dégrader les performances.
Ce dispositif a malheureusement un coup assez élevé (environ 90e) et consomme beaucoup d'énergie (500mA).
